...
import xlsxwriter
import urllib.parse
from fpdf import FPDF # NECESITA ESTAR EN REQUIREMENTS.TXT
...
def check_password():
    ...
    with c2:
        ...
        # CAMBIO: Ahora pide Usuario Y Contraseña
        ...
        if st.button("Entrar / Login", type="primary"):
            ...
            if user_clean in USUARIOS and USUARIOS[user_clean] == pass_clean:
                ...
                st.session_state.username = user_clean # Guardamos quién es
                st.rerun()
            else:
...
TR = {
    ...
    "English": {
        ...
        "val_map": {"NEW": "🆕 New", "VENTA": "💰 Sale", "EDITAR": "✏️ Edit", "BORRAR": "🗑️ Del", "BORRADO_MASIVO": "🔥 Bulk", "CREAR": "✨ Create", "HIST_DEL": "🧹 Clean", "META_UPDATE": "🎯 Goal"}
    }
}
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("📲 Instalar App"):
            st.info(TR[lang].get("install_guide", "Menu -> Add to Home Screen"))

        st.markdown("---")
        st.caption("v39.0 Enterprise")
    ...
    with tab_dash:
        ...
        if not df.empty:
            ...
            for prod_name, sold_kg in stock_v.items():
                # Simulamos un límite de 1000kg (puedes cambiar esto en el código)
                LIMIT_KG = 1000.0
        ...
        with st.container(border=True):
            ...
            if st.button(t['forms'][4], type="primary"):
                if emp and prod:
                    ...
                    time.sleep(5) # Damos tiempo para bajar el PDF antes de recargar
                    st.rerun()
